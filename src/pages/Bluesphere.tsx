...
const ENGAGEMENT_ITEMS = [
  {
    ...
    fullDescription: 'Pulse Rush is our signature mini-game where you navigate through digital waves to collect BlueSea tokens.',
    type: 'game',
  ...
  },
  {
    ...
    fullDescription: 'The SpinVault is where luck meets rewards. Every 24 hours, you get one free spin.',
    type: 'spin',
  ...
  },
  {
    ...
    fullDescription: 'We value your voice. Participate in our weekly polls to decide which new services to prioritize.',
    type: 'poll',
  ...
  },
  {
    ...
    fullDescription: 'Sharpen your financial literacy with BlueSphere Trivia. Answer 5 quick questions correctly.',
    type: 'question',
  ...
  return (
    ...
    >
      <div className="p-5 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-4 leading-relaxed">{item.description}</p>
      ...
      <div className={cn("px-5 overflow-hidden transition-all duration-300 ease-in-out", isExpanded ? "max-h-[500px] pb-6 opacity-100" : "max-h-0 opacity-0")}>
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-4">
          <p className="text-sm text-slate-600 dark:text-slate-300">{item.fullDescription}</p>
          ...
          <button onClick={() => navigate(item.route)} className="w-full mt-2 py-3.5 rounded-xl bg-sky-500 text-white font-bold text-sm flex items-center justify-center gap-2 hover:bg-sky-600 transition-all shadow-lg shadow-sky-500/20 active:scale-[0.98]">
            {item.ctaText}
        ...
        <main className="flex-1 p-4 md:p-6 overflow-y-auto">
          <div className="max-w-5xl mx-auto space-y-6">
            <div className="relative">
              ...
              <Input type="text" placeholder="Find a game..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-12 py-6 rounded-xl border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800 shadow-sm" />
            </div>

            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((category) => (
                <button key={category} onClick={() => setActiveCategory(category)} className={cn('px-6 py-2.5 rounded-full text-sm font-bold transition-all', activeCategory === category ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/30' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-slate-800')}>
                  {category}
                </button>
              ))}
            </div>
