...
export function Pension() {
  ...
  const handleCreate = () => {
    if (!form.name || !form.monthlyContribution) { showToast('Please fill all fields', true); return; }
  ...
  const handleContribute = (planId: string) => {
    const amount = Number(contributionAmount);
    if (!amount || amount <= 0) return;
    contributePension(planId, amount);
    addTransaction({ transaction_type: 'DEBIT', amount, description: `Pension Contribution`, status: 'successful', category: 'pension', payment_method: 'Wallet' });
          ...
          <div className="bg-slate-900 dark:bg-slate-800 rounded-3xl p-6 text-white shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-3xl font-black mt-1">₦{totalPension.toLocaleString()}</p>
              </div>
            ...
            <div className="flex gap-4 text-[10px] text-slate-400">
              <span>{pensionPlans.length} Active Plans</span>
              <span>•</span>
              <span>Projected: ₦{(pensionPlans.reduce((s, p) => s + p.projectedGrowth, 0)).toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-3xl p-6 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={pensionGrowthData}>
                <defs><linearGradient id="penGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f59e0b" stopOpacity={0.3}/><stop offset="95%" stopColor="#f59e0b" stopOpacity={0}/></linearGradient></defs>
              ...
              pensionPlans.map(plan => (
                <div key={plan.id} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => setShowContribute(plan.id)} className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-[10px] font-bold transition-all active:scale-95">
                      Contribute
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-xl bg-slate-950/40">
          ...
          <div className="relative bg-white dark:bg-slate-900 sm:rounded-[2rem] rounded-t-[2rem] p-6 w-full max-w-lg shadow-2xl border-t sm:border border-slate-200 dark:border-white/10 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <Button onClick={handleCreate} disabled={!form.name || !form.monthlyContribution} className="w-full bg-amber-500 hover:bg-amber-600 text-white h-14 rounded-2xl text-sm font-black shadow-xl disabled:opacity-50 active:scale-95">Create Plan</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {showContribute && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 backdrop-blur-xl bg-slate-950/40">
          ...
          <div className="relative bg-white dark:bg-slate-900 sm:rounded-[2rem] rounded-t-[2rem] p-6 w-full max-w-sm shadow-2xl border-t sm:border border-slate-200 dark:border-white/10">
            ...
            <Button onClick={() => handleContribute(showContribute)} disabled={!contributionAmount} className="w-full bg-amber-500 hover:bg-amber-600 text-white h-14 rounded-2xl text-sm font-black shadow-xl disabled:opacity-50 active:scale-95">Contribute Now</Button>
          </div>
        </div>
      )}
      {ToastComponent}
