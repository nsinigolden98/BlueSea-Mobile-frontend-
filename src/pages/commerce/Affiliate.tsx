...
// Strongly typed definitions for cleaner backend API contract structures
export interface AffiliateItem {
  id: string;
  type: 'product' | 'event' | 'service' | 'property';
  title: string;
  commission: number;
  price: number;
  seller: string;
...
const AFFILIATE_ITEMS: AffiliateItem[] = [
  { id: 'a1', type: 'product', title: 'Premium Sneakers', commission: 15, price: 45000, seller: 'StyleHub Store', clicks: 234, conversions: 18 },
  { id: 'a2', type: 'event', title: 'Tech Summit 2026', commission: 10, price: 25000, seller: 'BlueSea Events', clicks: 567, conversions: 42 },
  { id: 'a3', type: 'service', title: 'Logo Design Package', commission: 20, price: 35000, seller: 'Creative Studio', clicks: 189, conversions: 12 },
  { id: 'a4', type: 'property', title: 'Luxury Apartment Lekki', commission: 5, price: 5000000, seller: 'Prime Realty', clicks: 89, conversions: 3 },
  { id: 'a5', type: 'product', title: 'Wireless Earbuds Pro', commission: 12, price: 25000, seller: 'TechWorld', clicks: 445, conversions: 31 },
  { id: 'a6', type: 'event', title: 'Afrobeats Festival', commission: 8, price: 15000, seller: 'Lagos Events', clicks: 892, conversions: 67 },
];
...
export function Affiliate() {
  ...
  const handleCopy = async (itemId: string) => {
    ...
    try {
      ...
      // await fetch('/api/affiliate/log-share', { method: 'POST', body: JSON.stringify({ itemId }) });
    } catch (err) {
      showToast('Failed to copy link');
    }
  };

  // Dedicated action handler using the Share2 and Button component
  const handleNativeShare = async (item: AffiliateItem) => {
    const shareData = {
      title: item.title,
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col">
      ...
      <main className="flex-1 p-4 md:p-6 overflow-y-auto scrollbar-hide">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-slate-900 dark:bg-slate-800 rounded-3xl p-6 text-white shadow-xl">
            ...
            {/* Stats grid using MousePointer, Users, and TrendingUp icons */}
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 bg-white/5 rounded-2xl flex flex-col items-center justify-center relative group">
                <MousePointer className="w-3.5 h-3.5 text-sky-400 absolute top-2 right-2 opacity-60" />
                <p className="text-lg font-black mt-1">{totalClicks.toLocaleString()}</p>
                <p className="text-[9px] text-slate-400">Clicks</p>
              </div>
              <div className="p-3 bg-white/5 rounded-2xl flex flex-col items-center justify-center relative group">
                <Users className="w-3.5 h-3.5 text-purple-400 absolute top-2 right-2 opacity-60" />
                <p className="text-lg font-black mt-1">{totalConversions}</p>
                <p className="text-[9px] text-slate-400">Conversions</p>
              </div>
              <div className="p-3 bg-white/5 rounded-2xl flex flex-col items-center justify-center relative group">
                <TrendingUp className="w-3.5 h-3.5 text-emerald-400 absolute top-2 right-2 opacity-60" />
                <p className="text-lg font-black mt-1">{((totalConversions / totalClicks) * 100).toFixed(1)}%</p>
              ...
              return (
                <div key={item.id} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {/* Replaced raw markup button with the customized shared button component wrapper */}
                    <div className="flex items-center gap-2">
                      <Button 
                        variant="ghost" 
                        size="icon" 
                        onClick={() => handleNativeShare(item)}
                        className="h-8 w-8 text-slate-500 dark:text-slate-400 hover:text-emerald-500 rounded-xl bg-slate-100 dark:bg-slate-800 border-none"
                      >
                        <Share2 className="w-3.5 h-3.5" />
                      </Button>
                      <button 
                        onClick={() => handleCopy(item.id)} 
                        className="px-4 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-emerald-500 hover:text-white text-slate-600 dark:text-slate-300 rounded-xl text-[10px] font-bold transition-all active:scale-95 flex items-center gap-1.5"
                      >
      ...
      {/* Dynamic invocation check ensures total compatibility with your internal custom hook models */}
