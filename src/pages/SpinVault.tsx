...
export function Rewards() {
  ...
  const [showResultModal, setShowResultModal] = useState({ show: false });
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setAirtimeBalance(data.airtimeBalance);
  ...
  const canSpin = () => {
    ...
    const now = Date.now();
    return now - lastSpinTime >= 86400000;
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 w-full bg-white/80 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <button 
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-slate-600 dark:text-slate-400 hover:text-sky-500 dark:hover:text-sky-400 transition-colors group"
          >
            <div className="p-2 rounded-xl bg-slate-100 dark:bg-slate-900 group-hover:bg-sky-50 dark:group-hover:bg-sky-900/30 transition-colors">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">Active Balance</span>
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex items-center gap-12 px-6">
              <span className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider">
                <Sparkles className="w-3.5 h-3.5 text-sky-400" /> User_72{i} won ₦500 Airtime
              </span>
              <span className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider">
                <Zap className="w-3.5 h-3.5 text-amber-400" /> New 7-Day Streak Achieved
              </span>
              <span className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider">
                <CircleDot className="w-3.5 h-3.5 text-emerald-400" /> Mystery Box Opened
              </span>
            </div>
          ))}
        </div>
...
<main className="flex-1 w-full max-w-5xl mx-auto p-4 md:p-8 space-y-8">
            ...
            <div className="relative z-10 flex flex-col items-center">
              <div className="w-full flex justify-between items-start mb-8">
                <div>
                  ...
                  <p className="text-slate-400 text-sm font-medium">Try your luck once every 24 hours</p>
                </div>
                <div className="px-4 py-1.5 bg-sky-500/10 text-sky-500 rounded-full text-[10px] font-black uppercase">Live Reward</div>
              </div>
              ...
              <div className="relative w-72 h-72 md:w-80 md:h-80 mb-10 group">
                ...
                >
                  {SECTORS.map((s, i) => (
                    ...
                    >
                      <span className="absolute top-10 left-14 -rotate-90 origin-center text-[11px] font-black uppercase tracking-tighter text-slate-900 dark:text-white">
                        {s.label}
                      </span>
                    </div>
                  ))}
                ...
                <div className="absolute inset-0 m-auto w-14 h-14 bg-slate-900 rounded-full border-4 border-white dark:border-slate-700 z-20 flex items-center justify-center shadow-2xl">
              ...
              <div className="w-full max-w-sm space-y-4">
                <Button 
                  ...
                  className="w-full h-16 text-lg font-black rounded-2xl shadow-xl shadow-sky-500/20 bg-sky-500 hover:bg-sky-600 dark:bg-sky-600 dark:hover:bg-sky-500 text-white transition-all active:scale-95 disabled:bg-slate-200 dark:disabled:bg-slate-800"
                >
                  {isSpinning ? (
                    ...
                    <span className="flex items-center gap-2"><Clock className="w-5 h-5" /> {getTimeRemaining()}</span>
                  )}
                </Button>
          ...
          <div className="w-full lg:w-80 space-y-6">
            <div className="bg-indigo-600 rounded-[2rem] p-6 text-white relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-[10px] text-indigo-100 uppercase font-black mb-6 tracking-widest">Locked Reward</p>
                ...
                <div className="h-2.5 bg-black/20 rounded-full overflow-hidden">
                  <div className="h-full bg-white transition-all duration-1000" style={{ width: `${(Math.min(streak, 7)/7)*100}%` }} />
                </div>
              </div>
              ...
              <div className="space-y-4 max-h-[280px] overflow-y-auto no-scrollbar">
                {history.length > 0 ? history.map((item) => (
                  <div key={item.id} className="flex items-center justify-between group">
                    ...
                    <ArrowUpRight className="w-3 h-3 text-slate-300 group-hover:text-sky-500 transition-colors" />
                  </div>
                )) : (
                  <div className="py-8 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">No wins recorded</div>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button onClick={() => setShowDevPanel(!showDevPanel)} className="w-12 h-12 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-full flex items-center justify-center shadow-2xl">
        ...
        {showDevPanel && (
          <div className="absolute bottom-16 right-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-2xl w-48 space-y-2">
            <button onClick={() => setLastSpinTime(prev => prev - 86400000)} className="w-full text-left p-2 text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg">Simulate Next Day</button>
            <button onClick={() => setBonusSpins(s => s + 1)} className="w-full text-left p-2 text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg">Add Bonus Spin</button>
            <button onClick={() => { setPoints(120); setAirtimeBalance(0); setStreak(1); setHistory([]); setLastSpinTime(null); }} className="w-full text-left p-2 text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">Hard Reset</button>
          </div>
        )}
      </div>
      ...
      {showResultModal.show && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 w-full max-w-sm text-center shadow-2xl border border-white/10 relative overflow-hidden animate-in zoom-in-95">
            <div className="w-20 h-20 bg-sky-500/10 rounded-3xl flex items-center justify-center mx-auto mb-6">
              <Gift className="w-10 h-10 text-sky-500 animate-pulse" />
            </div>
            <h2 className="text-3xl font-black mb-2 tracking-tight">Reward Unlocked</h2>
            <p className="text-slate-400 text-sm mb-8 font-medium">You successfully redeemed</p>
...
function StatCard({ label, value, icon }) {
  return (
    <div className="bg-white dark:bg-slate-900 p-5 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-4">
      <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center shrink-0 border border-slate-100 dark:border-slate-800">
        {icon}
      </div>
      <div>
