  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-200 flex overflow-hidden transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-950 transition-colors duration-300">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">{branch.name}</span>
        ...
        <main className="flex-1 p-4 md:p-6 overflow-y-auto scrollbar-hide max-w-3xl w-full mx-auto pb-24">
          
          {/* Branch Meta Context Overview Matrix Card */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-4 mb-5 shadow-sm animate-slide-up">
            ...
            {branch.description && (
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-4 pt-3.5 border-t border-slate-100 dark:border-white/5 leading-relaxed">
                {branch.description}
              </p>
            )}
          </div>
          ...
          <div className="flex items-center gap-1.5 border-b border-slate-100 dark:border-white/5 pb-2 mb-4">
            {tabs.map(tab => {
            ...
            <div className="space-y-3 animate-slide-up">
              {payrolls.length > 0 ? (
                payrolls.map(payroll => (
                  <div key={payroll.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex items-center justify-between hover:border-sky-500/30 transition-all shadow-sm">
                    <div>
          ...
          {activeTab === 'settings' && (
            <div className="space-y-4 animate-slide-up">
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl divide-y divide-slate-100 dark:divide-white/5 overflow-hidden shadow-sm">
                <button className="w-full flex items-center justify-between p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-950/50 transition-colors group">
                  <div>
