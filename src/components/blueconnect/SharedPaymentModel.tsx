...
export const SharedPaymentModal: React.FC<SharedPaymentModalProps> = ({
 ...
 // const [activeStep, setActiveStep] = useState<'verify' | 'payment'>('verify');
  ...
  useEffect(() => {
    if (message) {
      if (message.success) {
        handleExecutePayment(message.pin || '1234');
      } else {
        showToast(message.message || 'Authentication cancelled.');
      }
    }
  }, [message]);
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-1">
                {config.verificationField.label}
              </Label>
              ...
              {verificationError && (
                <p className="text-[10px] text-red-500 font-black ml-1 animate-shake">
                  {verificationError}
                </p>
              )}
            </div>
