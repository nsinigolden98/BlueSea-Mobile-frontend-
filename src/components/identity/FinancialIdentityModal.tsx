...
export const FinancialIdentityModal: React.FC<FinancialIdentityModalProps> = ({
  ...
  const handleSubmitVerification = async () => {
    ...
    try {
      ...
      const selectedBankCodeValue = String(selectedBankCode);

      const payload = {
        first_name: userFullName.split(' ')[0] || '',
        last_name: userFullName.split(' ').slice(1).join(' ') || '',
        account_number: accountNumber,
        bank_code: selectedBankCodeValue,
        bvn: encryptedBvn,
      };

      const response = await postRequest(`${API_BASE}/accounts/dva/assign/`, payload);

      if (!response || response?.state === false || response?.success === false) {
        throw new Error(response?.message || response?.detail || 'Financial identity verification could not be submitted.');
      }
      ...
      const profileResponse = await getRequest(`${API_BASE}/user_preference/user/`);
      ...
      if (profile?.has_DVA && profile?.dva_account?.account_number) {
        // Account creation is confirmed by the backend profile. The actual
        // DVA details remain sourced from that backend profile.
        return;
      }
    } catch (error: unknown) {
        ...
        <div className="p-6 max-h-[80vh] overflow-y-auto space-y-5">
          ...
          {step === 'review' && (
            <div className="space-y-5">
              ...
              {submitError && (
                <p className="text-[11px] text-red-500 bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/40 rounded-xl p-3">{submitError}</p>
              )}
