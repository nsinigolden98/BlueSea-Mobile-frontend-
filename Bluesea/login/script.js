...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("modal_tab_phone").style.borderBottomColor =
    "#ffffff";
...
function emailSwitch() {
  ...
  document.getElementById("modal_tab_phone").style.borderBottomColor =
    "#ffffff";
  document.getElementById("modal_tab_email").style.borderBottomColor =
    "#0ea5e9";
  document.getElementById("modal_tab_email").style.color = "#0ea5e9";
...
$$(".toggle_password").forEach((btn) => {
  safeAdd(btn, "click", () => {
    ...
    if (isPwd) btn.classList.add("toggle_show");
    else btn.classList.remove("toggle_show");
...
function startCountdown(spanEl, btnEl) {
  ...
  const id = setInterval(() => {
    ...
    if (t <= 0) {
      ...
      spanEl.textContent = OTP_RESEND_SECONDS;
    }
  }, 1000);
  return id;
}
...
async function verifyEmailSignUp() {
  ...
  if (!/^\d{6}$/.test(code)) {
    setError(modal_email_input, "Enter 6-digit code");
    return;
  } else {
...
async function signInButton(){
  ...
  let valid = true;
  if (!identifier) {
    setError($("#login_identifier"), "Please enter email");
    valid = false;
  }
...
async function SignUpButton() {
  ...
  if (!validateEmail(email)) {
    setError($("#signup_email"), "Please enter a valid email");
    valid = false;
  }
  if (!phoneRaw) {
    setError($("#signup_phone"), "Please enter your phone.");
    valid = false;
  }
  if (!name) {
    setError($("#signup_name"), "Please enter your name.");
    valid = false;
  }
  ...
  if (validateEmail(surname)) {
    setError($("#signup_surname"), "Please enter your surname.");
    valid = false;
  }
  if (!surname) {
    setError($("#signup_surname"), "Please enter you surname.");
    valid = false;
  }
  if (!password) {
    setError($("#signup_password"), "Please enter a password.");
    valid = false;
  }
  if (!confirm) {
    setError($("#signup_confirm"), "Confirm your password.");
    valid = false;
  }
  if (!terms) {
    setError($("#signup_terms"), "You must accept Terms & Policy.");
    valid = false;
  }
  if (!valid) return;
  ...
  if (!phone) {
    setError(
      $("#signup_phone"),
      "Use Nigerian format: 0XXXXXXXXXX or +234XXXXXXXXXX starting with 7|8|9."
    );
    return;
  }
